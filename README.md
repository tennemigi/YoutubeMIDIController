...
- ページ上のバッジから拡張機能の ON / OFF を切り替え
- ページ上のバッジから `Auto / Deck 1 / Deck 2` を明示選択
...
- `JOG 外周` で一時的な加速 / 減速
- `JOG 天面` でシーク
- バッジをクリックして拡張機能の ON / OFF を切り替え
- バッジをクリックしてこのウィンドウの Deck を `Auto / Deck 1 / Deck 2` から選択

## 操作内容

- `Play/Pause`
  - 再生と一時停止を切り替えます
  - 再生待機中は LED が点滅し、再生中は点灯します
- `Cue`
  - CUE 未保存時は現在位置を保存します
  - CUE 保存済み時は保存位置へ戻ります
  - CUE 保存済みのときは LED が点灯します
- `CH CUE`
...
- バッジから明示的に `Deck 1` または `Deck 2` を選ぶと、そのウィンドウを優先的にそのデッキへ固定できます
- `Auto` を選ぶと自動割り当てへ戻ります
